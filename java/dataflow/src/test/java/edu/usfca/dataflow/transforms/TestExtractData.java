...
public class TestExtractData {
  ...
  static ExtractAddicts PTransformAddicts1 = new ExtractData.ExtractAddicts("ZtPW19", 1);
  static ExtractAddicts PTransformAddicts2 = new ExtractData.ExtractAddicts("ZtPW19", 2);
  ...
  public void testHigh() {
    PCollection<PurchaserProfile> pp =
        tp.apply(Create.of(Arrays.asList(
            SAMPLES
        ))).apply(PTransformGetPP).apply(PTransformMerge);
...

    {
      ...
      PAssert.that(pp.apply(PT)).satisfies(out -> {
        ...
        for (DeviceId id : out) {
          DeviceId canonicalId = Utils.getCanonicalDeviceId(id);
